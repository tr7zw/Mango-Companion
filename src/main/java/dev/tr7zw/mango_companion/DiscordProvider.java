...
public class DiscordProvider {
    ...
    public void init() {
        if (App.getConfig().getDiscordApiToken() != null
                && !App.getConfig().getDiscordApiToken().isEmpty()
                && App.getConfig().getChannelId() != -1) {
            try {
                api =
                        new DiscordApiBuilder()
                                .setToken(App.getConfig().getDiscordApiToken())
                                .login()
                                .join();
                log.info("Url to add the bot to your Discord: " + api.createBotInvite());
                Optional<TextChannel> channel =
                        api.getTextChannelById(App.getConfig().getChannelId());
                if (channel.isPresent()) {
                    targetChannel = channel.get();
                } else {
                    throw new IllegalArgumentException(
                            "Discord channel not found! Is the bot on the correct server?");
                }
            } catch (Exception ex) {
