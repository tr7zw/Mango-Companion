...
public class MangoApiEndpoint extends AbstractRestResource<GsonWebSerialDeserial> {
    ...
    public ChapterResponse getChapters(@RequestParam(value = "url", required = false) String url) {
        ...
        Optional<Parser> matchingParser = App.getCompanion().getParsers().stream().filter(p -> p.canParse(url))
                .findFirst();
    ...
    public ChapterDataResponse selectChapter(@RequestParam(value = "id", required = false) String id) {
